...
export const users = mysqlTable('users', {
  id: bigint('id', { mode: 'number' }).primaryKey(),     // 雪花 ID (BigInt as number for JS safety if < 2^53, but ideally BigInt)
  // Changed mode: 'number' to 'bigint' if we want real BigInts in JS, but schema had 'number' or 'bigint'?
  // Original users table had mode: 'bigint'. Let's stick to BigInt to be safe, or number if we are sure.
  // Original file content: id: bigint('id', { mode: 'bigint' }).primaryKey()
  // So I will use { mode: 'bigint' } to match original.
  email: varchar('email', { length: 255 }).notNull().unique(), // 邮箱
  password: varchar('password', { length: 255 }),                // 密码
  name: varchar('name', { length: 255 }),                        // 昵称 (来自 Linux DO)
  avatar: varchar('avatar', { length: 500 }),                    // 头像 (来自 Linux DO)
  linuxDoId: varchar('linux_do_id', { length: 255 }).unique(),   // Linux DO ID
